budgetApp.dataDisplayList = (function() {
  let colors = ['#88d8b0', '#ffcc5c', '#ff6f69', '#ffeead', '#96ceb4',
    '#e1f7d5', '#ffbdbd', '#c9c9ff', '#ffffff', '#f1cbff']; // TODO: colors in one place
  let dataDisplayList = document.getElementById('data-display-list');
  let currencySign = '$';
  let data = null;

  function getDataPercentages(data) {
    let dataSum = data.reduce((total, datum) => total + parseInt(datum.value), 0);
  ...
  function draw() {
    ...
    data.forEach((data, index) => {
      ...
      percentageDisplay.textContent = (dataPercentages[index] * 100).toFixed(0) + '%';
      listItem.append(percentageDisplay);

      // colour circle
      var svg = d3.select('#' + itemId)
        .append('svg')
        .attr('width', 30)
        .attr('height', 30)
        .append('g')
